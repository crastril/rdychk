...
export default function HomeClient() {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden overflow-y-auto custom-scroll bg-black text-slate-100 font-display selection:bg-purple-500/30">
          ...
          <div className="relative z-10 max-w-4xl w-full px-6 pt-32 md:pt-40 pb-16 flex flex-col items-start md:items-center justify-start text-left md:text-center min-h-screen md:mx-auto">
            <div className="w-full flex-grow flex flex-col justify-start items-start md:items-center text-left md:text-center mt-2 md:mt-4">
              ...
              <div className="text-left md:text-center w-full order-2">
                ...
                <p className="text-sm text-white/45 font-medium mb-5 text-left md:text-center leading-snug max-w-sm md:mx-auto">
                  Créez un groupe avec vos amis et synchronisez vos sorties —{' '}
                  <span className="text-white/70">sans les 50 messages.</span>
                </p>
              </div>
              ...
              <div className={cn("w-full max-w-md relative mb-12 order-3", mode === 'remote' && 'pointer-events-none')}>
                {/* Hard shadow layer */}
                    ...
                    {step === 'name' ? (
                      ...
                      <div className="flex flex-col gap-3 animate-in fade-in slide-in-from-right duration-300">
                        <div className="flex items-center gap-2">
                          <MapPin className="w-4 h-4 shrink-0" style={{ color: '#9c3030' }} weight="fill" />
                          <p
                            className="text-sm font-black uppercase tracking-[0.2em]"
                            style={{ fontFamily: 'var(--font-barlow-condensed)', color: '#9c3030' }}
                          >
                        ...
                        <div className="relative">
                          ...
                          />
                          {isSearchingLocation && (
                            <CircleNotch className="absolute right-4 top-4 w-5 h-5 animate-spin" style={{ color: '#9c3030' }} />
                          )}
                        </div>
              ...
              {/* In-Person History */}
              {user && mode === 'in_person' && (
                <div className="w-full max-w-md pb-8 order-4">
                  <p
                    ...
                    {recentGroups.length > 0 ? recentGroups.map(group => (
                      <Link key={group.slug} href={`/group/${group.slug}`}>
                        ...
                        >
                          ...
                          >
                            <MapPin className="w-4 h-4" style={{ color: '#9c3030' }} weight="fill" />
                          </div>
        ...
        <div className={cn(
          ...
          <div className="relative z-10 max-w-4xl w-full px-6 pt-32 md:pt-40 pb-16 flex flex-col items-end md:items-center justify-start text-right md:text-center min-h-screen ml-auto md:mx-auto">
            <div className="w-full flex-grow flex flex-col justify-start items-end md:items-center text-right md:text-center mt-2 md:mt-4">
              ...
              <div className="text-right md:text-center w-full order-2">
                ...
                <div className="relative w-full text-right md:text-center mt-1 mb-2 inline-block">
                ...
                <p className="font-mono text-sm text-purple-400/60 mb-5 text-right md:text-center leading-snug max-w-xs md:max-w-sm ml-auto md:mx-auto">
                  {'// créez un groupe avec vos amis et synchronisez vos sessions —'}{' '}
                  <span className="text-purple-300/80">sans les 50 messages.</span>
                </p>
              </div>

              {/* Exact height matching container for animated Discord chat */}
              <div className="w-full h-[200px] md:h-[240px] relative max-w-lg md:mx-auto overflow-hidden text-right mb-6 px-2 md:px-0 flex flex-col justify-end order-1"
                style={{ WebkitMaskImage: 'linear-gradient(to bottom, transparent 0%, black 35%)', maskImage: 'linear-gradient(to bottom, transparent 0%, black 35%)' }}>
              ...
              {/* Online Form */}
              <div className={cn("group relative w-full max-w-md flex flex-col gap-4 order-3", mode === 'in_person' && 'pointer-events-none')}>
              ...
              {/* Online History */}
              {user && mode === 'remote' && (
                <div className="mt-8 w-full max-w-2xl flex flex-col items-end md:items-center pb-8 order-4">
      ...
      {/* =========================================================
          COMMENT ÇA MARCHE — Section scrollable
          ========================================================= */}
      <div className={cn(
        "relative w-full py-20 px-6 transition-colors duration-700",
        mode === 'remote' ? 'bg-[#0e0514]' : 'bg-black'
      )}>
        {/* Séparateur */}
        <div className="absolute top-0 left-0 w-full h-[2px]" style={{
          background: mode === 'remote'
            ...
            : 'linear-gradient(90deg, transparent, rgba(255,46,46,0.4), transparent)'
        }} />
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-12 text-center">
            {mode === 'remote' ? (
              <p className="font-mono text-xs text-purple-500/60 uppercase tracking-[4px] mb-2">{'// how_it_works'}</p>
            ) : (
              <p
                className="text-xs font-black uppercase tracking-[4px] mb-2"
                style={{ fontFamily: 'var(--font-barlow-condensed)', color: 'rgba(255,46,46,0.6)' }}
              >
                Comment ça marche
              </p>
            )}
            <h2 className={cn(
              ...
              mode === 'remote'
                ? 'font-mono text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-500'
                : 'text-white'
            )}>
              {mode === 'remote' ? 'TROIS_ÉTAPES_SUFFIT' : 'EN 3 ÉTAPES'}
            </h2>
          </div>
            ...
            {(mode === 'remote' ? [
              {
                num: '01',
                title: 'INIT_SESSION',
                desc: 'Nomme ton event — Raid, Ranked, Tournoi... et configure ce que le groupe doit décider.',
              },
              {
                num: '02',
                title: 'INVITE_TEAM',
                desc: 'Partage le lien. Chaque membre rejoint en un clic, aucun compte requis.',
              },
              {
                num: '03',
                title: 'VOTE_&_PLAY',
                desc: 'Tout le monde vote pour la date et le jeu. La session se cale automatiquement.',
              },
            ] : [
              {
                num: '01',
                title: 'Crée ton groupe',
                desc: 'Nomme ton événement — Barbecue, Ciné, Match... et choisis ce que le groupe doit décider.',
              },
              {
                num: '02',
                title: 'Invite tes potes',
                desc: 'Partage le lien. Chaque membre rejoint en un clic, sans inscription.',
              },
              {
                num: '03',
                title: 'Tout le monde vote',
                desc: 'Chacun vote pour la date et le lieu. Résultat immédiat — fini les 50 messages.',
              },
            ]).map((step, i) => (
              <div
                key={step.num}
              ...
              >
                ...
                <span
                  className={cn(
                    ...
                    mode === 'remote'
                      ? 'font-mono text-purple-500/20'
                      : ''
                  )}
                  style={mode === 'in_person' ? {
                    fontFamily: 'var(--font-barlow-condensed)',
                    color: '#9c3030',
                    opacity: 0.25,
                  } : {}}
                ...
                <p
                  className={cn(
                    ...
                    mode === 'remote' ? 'font-mono text-sm uppercase tracking-[0.1em] text-purple-200' : 'text-xl'
                  )}
                  style={mode === 'in_person' ? { fontFamily: 'var(--font-barlow-condensed)', fontSize: '1.25rem', letterSpacing: '0.04em' } : {}}
                >
                  {step.title}
                </p>
                {/* Description */}
                <p className={cn(
                  "leading-relaxed",
                  mode === 'remote' ? 'font-mono text-xs text-purple-400/60' : 'text-sm text-white/40 font-medium'
                )}>
                  {mode === 'remote' && <span className="text-purple-600/60">{'// '}</span>}{step.desc}
                </p>
              </div>
            ))}
          </div>
